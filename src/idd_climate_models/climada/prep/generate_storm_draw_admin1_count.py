"""
Generate storm_draw_admin1_count.parquet
...
For each (storm_draw, source_id, variant_label, experiment_id, batch_year, basin) task:
  ...
  - num_admin1_first_year: unique FHS admin1 regions intersecting the first-year PAF raster
  - num_years_in_batch / estimated_admin1_total: derived

The admin1 shapefile is pre-filtered to FHS hierarchy locations only
(global_WGS84_admin1_0_360.parquet, produced by generate_0_360_shapefiles.py).
Intersection with the PAF raster therefore counts only FHS admin1 regions
that a storm actually touches — storms that don't reach any FHS admin1
polygon produce a count of 0 but are still included in the output.

Output: /mnt/team/rapidresponse/pub/tropical-storms/climada/output/storm_draw_admin1_count.parquet
"""
...
def process_row(row: pd.Series) -> list[dict[str, Any]]:
    ...
    num_admin1_first_year = intersected["ADM1_CODE"].nunique()
    ...
    return [{
        **params,
        ...
        "year": str(start_year),
        "num_admin1_first_year": num_admin1_first_year,
        "num_years_in_batch": num_years,
        "estimated_admin1_total": num_admin1_first_year * num_years,
    }]
