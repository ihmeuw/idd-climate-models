"""
orchestrate_dh_only.py
Jobmon orchestrator for DH-only stage fitting (201 stages).

Run with:
    python src/idd_climate_models/tc_models/orchestrate_dh_only.py
"""
...
from idd_climate_models.tc_models.stage_grid_dh_only import build_stage_grid
from idd_climate_models.tc_models.cache import stage_to_id, is_stage_logged
...
def main():
    ...
    # ── Jobmon workflow ────────────────────────────────────────────────────────
    tool = Tool(name='tc_dh_stages')
    wf   = tool.create_workflow(
        name=f'tc_dh_stages_{uuid.uuid4()}',
    ...
    template = tool.get_task_template(
        ...
        command_template=(
            ...
            f'export STAGE_RESULTS_DIR={RESULTS_DIR} && '
            f'python {WORKER} '
