...
class Coupled_FAST(bam_track.BetaAdvectionTrack):
    def __init__(self, fn_wnd_stat, basin, dt_start, namelist_dict):
        dt_s = namelist_dict['output_interval_s']
        total_time_s = namelist_dict['total_track_time_days'] * 24 * 60 * 60
        super().__init__(fn_wnd_stat, basin, dt_start, namelist_dict, dt_s, total_time_s)

        """ FAST Constants """
        self.Ck = namelist_dict['Ck']                       # surface enthalpy coefficient
        self.h_bl = namelist_dict['atm_bl_depth']           # boundary layer depth (m)
        ...
        self.f_bath = geo.read_bathy(basin, namelist_dict)
        self.f_land = geo.read_land(basin, namelist_dict)
        self.debug = False

        # Variable drag coefficient
        self.f_Cd = geo.read_drag(basin, namelist_dict)
    ...
    def _calc_env_wnd_to_shr(self, env_wnds):
        u250, v250, u850, v850 = env_wind.deep_layer_winds(np.expand_dims(env_wnds, axis = 0), self.namelist_dict)
    ...
    def _calc_steering_coefs(self, v):
        ...
        if self.namelist_dict['coupled_track']:
            alpha_fx = (v*1.94384)*np.array(self.namelist_dict['m_alpha']) + np.array(self.namelist_dict['y_alpha'])
            steering_coefs = np.maximum(np.minimum(alpha_fx, self.namelist_dict['alpha_max']), self.namelist_dict['alpha_min'])
            if np.any(np.isnan(steering_coefs)):
                steering_coefs = np.array(self.namelist_dict['y_alpha'])
        else:
