...
def calc_T_rho(T,rv,rt,config_dict):
    if config_dict['select_thermo'] == 1:
        T_rho = T*(1+rv/pr.eps)/(1+rv)
    elif config_dict['select_thermo'] == 2:
...
def CAPE_PI(sst,p_surf,p_env,T_env,r_env,config_dict):
    # Ratio of the exchange coefficients
    cecd = config_dict['Ck'] / config_dict['Cd'];
    ...
    select_interp = config_dict['select_interp']
    # If interpolation, load temperature table and coordinates
    if config_dict['select_interp'] == 2:
        if select_thermo == 1:
            with np.load('%s/thermo/entropy_table.npz' % config_dict['src_directory']) as entropy_table:
                p_look = entropy_table['p']
                s_look = entropy_table['s']
                T_lookup = entropy_table['T']
        elif select_thermo == 2:
            with np.load('%s/thermo/entropy_table_reversible.npz' % config_dict['src_directory']) as entropy_table:
                p_look = entropy_table['p']
                s_look = entropy_table['s']
    ...
    T_rho_env = calc_T_rho(T_env,r_env,r_env,config_dict)
    ...
    for hh in np.arange(dim_in[1]):
        for gg in np.arange(dim_in[2]):
            ...
            T_rho_a[:,hh,gg] = calc_T_rho(Ta_prof[:,hh,gg],ra_prof[:,hh,gg],r_ns[hh,gg],config_dict)
            T_rho_s[:,hh,gg] = calc_T_rho(Ts_prof[:,hh,gg],rs_prof[:,hh,gg],rs[hh,gg],config_dict)
...
Output:
Potential intensity [m/s]"""
def CAPE_PI_vectorized(sst,p_surf,p_env,T_env,r_env,config_dict):
    # Ratio of the exchange coefficients
    cecd = config_dict['Ck'] / config_dict['Cd'];
    # Select thermodynamics: Set to 1 for pseudoadiabatic, 2 for reversible
    select_thermo = config_dict['select_thermo']
    ...
    # Compute density temperature. Assume that there is no liquid or ice water at lower level.
    T_rho_a = calc_T_rho(Ta_prof,ra_prof,r_ns,config_dict)
    T_rho_s = calc_T_rho(Ts_prof,rs_prof,rs,config_dict)
    # Compute CAPE differences
    # Find LNB for saturated and BL profiles
