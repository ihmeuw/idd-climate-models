...
class BetaAdvectionTrack:
    ...
    """
    def __init__(self, fn_wnd_stat, basin, dt_start, namelist_dict, dt_track = 3600,
                 total_time = 15*24*60*60):
        self.fn_wnd_stat = fn_wnd_stat
        self.namelist_dict = namelist_dict  # Store config dict
        ...
        self.T_Fs = namelist_dict['T_days']*24*60*60    # 15-day period of the fourier series
        self.u_beta = namelist_dict['u_beta']           # zonal beta drift speed
        self.v_beta = namelist_dict['v_beta']           # meridional beta drift speed
        self.nLvl = len(namelist_dict['steering_levels'])
        self.nWLvl = self.nLvl * 2
        self.dt_start = dt_start.astype('datetime64[ns]')
        self.basin = basin
        self.var_names = env_wind.wind_mean_vector_names(namelist_dict)
    ...
    def _load_wnd_stat(self):
        wnd_Mean, wnd_Cov = env_wind.read_env_wnd_fn(self.fn_wnd_stat, self.namelist_dict)
    ...
    def _env_winds(self, clon, clat, ts):
        ...
        try:
            # 1. Attempt original Cholesky decomposition
            wnd_A = np.linalg.cholesky(wnd_cov)
        except np.linalg.LinAlgError:
            # 2. If decomposition fails, fix the matrix using the imported function
            wnd_cov_fixed = mat.nearestPD(wnd_cov)
            
            try:
                # 3. Attempt Cholesky decomposition on the fixed matrix
                wnd_A = np.linalg.cholesky(wnd_cov_fixed)
            except np.linalg.LinAlgError as err_fixed:
                # 4. If it STILL fails (highly unlikely), log and return zeros
                print('FATAL ERROR: Cholesky failed even after nearestPD.')
                print('Original Covariance matrix:\n', wnd_cov)
                print('Fixed Covariance matrix:\n', wnd_cov_fixed)
                print('Datetime start:', self.dt_start)
                return np.zeros(self.nWLvl)
            
        # Use the fixed matrix to generate the winds if it was corrected, otherwise 
        # use the originally computed wnd_A.
        # Note: If wnd_A was successfully calculated in the except block, 
        # the code jumps back here.
        wnds = wnd_mean + np.matmul(wnd_A, self.Fs_i(ts))
        return wnds
    
    # This is the original version that *did not* use the already defined nearestPD function
    # """ Calculate environmental winds at a point and time. """
    # def _env_winds(self, clon, clat, ts):
    #     if np.isnan(clon) or np.isnan(ts):
    #         return np.zeros(self.nWLvl)
    ...
    #         eigvals = np.linalg.eigvalsh(wnd_cov)
    #         print("Eigenvalues:", eigvals)
    #         print('Cholesky decomposition failed at:', err)
    ...
    def _calc_steering_coefs(self):
        assert len(self.namelist_dict['steering_coefs']) == len(self.namelist_dict['steering_levels'])
        steering_coefs = np.array(self.namelist_dict['steering_coefs'])
