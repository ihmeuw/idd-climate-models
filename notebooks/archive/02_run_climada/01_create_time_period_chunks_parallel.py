...
# --- Template 2: Process ALL DRAWS for a Basin (Modified 1b) ---
# This now processes ALL draws in a single job instead of one draw per job
basin_processor_template = tool.get_task_template(
    template_name="process_basin_all_draws",
    default_cluster_name="slurm", 
    default_compute_resources={ 
        "memory": "80G",  # Increased since we're processing multiple draws
        "cores": rfc.tc_risk_n_procs + 1, 
        "runtime": "12h",  # Increased for processing all draws
        "queue": 'long.q', 
        "project": project,
    },
    command_template=(
        "python {script_root}/1b_process_basin_all_draws.py "
        "--input_data_type {{input_data_type}} "
        "--input_io_data_type {{input_io_data_type}} "
        "--output_data_type {{output_data_type}} "
        "--output_io_data_type {{output_io_data_type}} "
    ...
    node_args=["input_data_type", "input_io_data_type", "output_data_type", "output_io_data_type", "data_source", "model", "variant", "scenario", "time_period", "basin", "num_draws"],
)
...
for _, row in unique_combos.iterrows():
    ...
    for basin in BASINS:
        ...
        # --- LEVEL 2: PROCESS ALL DRAWS FOR THIS BASIN (Single Task) ---
        basin_processor_task = basin_processor_template.create_task(
            input_data_type=INPUT_DATA_TYPE,
            input_io_data_type=INPUT_IO_DATA_TYPE,
            output_data_type=OUTPUT_DATA_TYPE,
            output_io_data_type=OUTPUT_IO_DATA_TYPE,
        ...
        all_tasks.append(basin_processor_task)
        dependencies.append((basin_processor_task, folder_task))


# --- ADD TASKS AND BIND DEPENDENCIES ---
workflow.add_tasks(all_tasks)
print(f"Total tasks created: {len(all_tasks)}")
print(f"  - Folder creation tasks: {len([t for t in all_tasks if 'create_input_folders' in str(t)])}")
print(f"  - Basin processor tasks: {len([t for t in all_tasks if 'process_basin_all_draws' in str(t)])}")
